...
光标在句首第一个字母V时，`iw`表示Vim三个字母，而`aw`表示Vim加上后面的空格
另外，`i(`表示括号内的内容，即screen-based，而`a(`表示包括两端的括号，即
(screen-based).
...
尽量使你的命令更模块化，具有清晰的含义与作作用范围，以便于与`.` 等命令协同
...
但是，当熟练后，尽可能直接使用完整的命令进行操作，因为相比于 `viwd`，`diw`
更加模块化，也更容易进行重复操作
